...
start_cleanup() {
    ...
    if [[ "${MOLE_GUI_SYSTEM_CACHES:-}" == "skip" ]]; then
        SYSTEM_CLEAN=false
        echo "  ${ICON_LIST} System-level cleanup skipped"
        echo "  ${ICON_LIST} User-level cleanup enabled"
        echo ""
    elif [[ "${MOLE_GUI_SYSTEM_CACHES:-}" == "include" ]]; then
        if ensure_sudo_session "System cleanup requires admin access"; then
            SYSTEM_CLEAN=true
            echo -e "${GREEN}${ICON_SUCCESS}${NC} Admin access granted"
        else
            SYSTEM_CLEAN=false
            echo -e "${YELLOW}${ICON_WARNING}${NC} Admin access not granted, continuing with user-level cleanup"
        fi
        echo ""
    elif [[ -t 0 ]]; then
        if adopt_sudo_session; then
...
main() {
    case "${1:-}" in
        "--gui-whitelist-list")
            ...
            mole_gui_whitelist_catalog "clean"
            exit $?
            ;;
        "--gui-whitelist-save")
            shift
            source "$SCRIPT_DIR/../lib/manage/whitelist.sh"
            mole_gui_whitelist_save "clean" "$@"
            exit $?
            ;;
    esac
