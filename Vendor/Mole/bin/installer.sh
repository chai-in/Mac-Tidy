...
collect_installers() {
    ...
    if [[ ${#all_files[@]} -eq 0 ]]; then
        if [[ "${IN_ALT_SCREEN:-0}" != "1" && "${MOLE_GUI_INSTALLER_MODE:-}" == "" ]]; then
...
mole_gui_print_installers_json() {
    printf '['
    local i
    for ((i = 0; i < ${#INSTALLER_PATHS[@]}; i++)); do
        [[ $i -gt 0 ]] && printf ','
        printf '{"name":"%s","path":"%s","source":"%s","size":"%s","size_bytes":%s}' \
            "$(mole_gui_json_escape "${INSTALLER_NAMES[i]}")" \
            "$(mole_gui_json_escape "${INSTALLER_PATHS[i]}")" \
            "$(mole_gui_json_escape "${INSTALLER_SOURCES[i]}")" \
            "$(mole_gui_json_escape "$(bytes_to_human "${INSTALLER_SIZES[i]}")")" \
            "${INSTALLER_SIZES[i]}"
    done
    printf ']\n'
}

# Re-scan, match only exact selected paths, then enter the existing immutable
# delete-plan and final identity/size checks. Any stale selection refuses the
# complete action before the first mutation.
mole_gui_remove_installers() {
    local -a requested_paths=("$@")
    reset_installer_delete_results
    if [[ ${#requested_paths[@]} -eq 0 ]]; then
        echo "No installer files selected" >&2
        return 1
    fi

    local -a selected_indices=()
    local requested index selected_index
    for requested in "${requested_paths[@]}"; do
        local matched=false
        for ((index = 0; index < ${#INSTALLER_PATHS[@]}; index++)); do
            [[ "$requested" == "${INSTALLER_PATHS[index]}" ]] || continue
            local duplicate=false
            for selected_index in "${selected_indices[@]+"${selected_indices[@]}"}"; do
        ...
        if [[ "$matched" == "false" ]]; then
            record_installer_delete_failure "$requested" "not present in current scan"
        fi
    done

    if [[ $total_delete_failed -gt 0 ]]; then
        echo "Installer selection changed; rescan before removing files" >&2
        return "$INSTALLER_EXIT_INCOMPLETE"
    fi
    build_installer_delete_plan "${selected_indices[@]}" || return "$INSTALLER_EXIT_INCOMPLETE"
...
main() {
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            "--gui-list")
                gui_mode="list"
                export MOLE_GUI_INSTALLER_MODE="list"
                ;;
            "--gui-remove")
                gui_mode="remove"
                export MOLE_GUI_INSTALLER_MODE="remove"
                shift
                gui_paths=("$@")
                set --
                continue
    ...
    if [[ "$gui_mode" == "remove" ]]; then
        if [[ "${MOLE_DRY_RUN:-0}" != "1" && "${MOLE_GUI_CONFIRMED:-0}" != "1" ]]; then
            echo "Native-app confirmation is required before removing installer files" >&2
            return 1
        fi
        if ! collect_installers; then
            echo "Installer selection changed; no current candidates" >&2
            return 1
        fi
        local gui_status=0
        mole_gui_remove_installers "${gui_paths[@]}" || gui_status=$?
        show_summary
        [[ $gui_status -eq 0 ]] || return 1
        return 0
    fi
