...
perform_purge() {
    if [[ "${MOLE_GUI_PURGE_MODE:-}" == "list" ]]; then
        local gui_scan_status=0
        local gui_stats_dir="${XDG_CACHE_HOME:-$HOME/.cache}/mole"
        ensure_user_dir "$gui_stats_dir"
        MOLE_GUI_PURGE_JSON_EMITTED=0
        exec 3>&1
        clean_project_artifacts 1>&2 || gui_scan_status=$?
        if [[ "${MOLE_GUI_PURGE_JSON_EMITTED:-0}" != "1" ]]; then
            printf '[]\n' >&3
        fi
        exec 3>&-
        return "$gui_scan_status"
    fi
    ...
    clean_project_artifacts || clean_status=$?
...
main() {
    ...
    if [[ "$MOLE_GUI_PURGE_MODE" == "remove" && "${MOLE_DRY_RUN:-0}" != "1" && "${MOLE_GUI_CONFIRMED:-0}" != "1" ]]; then
        echo "Native-app confirmation is required before purging project artifacts" >&2
        return 1
    fi
