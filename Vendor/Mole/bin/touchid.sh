...
show_gui_status() {
    ...
    printf '{"supported":%s,"configured":%s,"location":"%s"}\n' \
        "$supported" "$configured" "$location"
}
...
enable_touchid() {
    ...
    if ! supports_touchid; then
        ...
        if [[ "${MOLE_GUI_CONFIRMED:-0}" != "1" ]]; then
...
main() {
    ...
    if [[ "$gui_status" == "true" ]]; then
        if [[ -n "$command" ]]; then
            log_error "Native status cannot be combined with another Touch ID action"
            return 1
        fi
        show_gui_status
        return 0
