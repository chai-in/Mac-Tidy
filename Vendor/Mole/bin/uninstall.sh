...
# Resolve native-app selections only by exact path from the fresh application
# inventory. Refuse the complete selection when any requested path is stale.
match_apps_by_gui_path() {
    local -a requested_paths=("$@")
    local requested_path app_data app_path selected_app existing_path
    selected_apps=()

    for requested_path in "${requested_paths[@]}"; do
        local matched=false
        for app_data in "${apps_data[@]+"${apps_data[@]}"}"; do
            IFS='|' read -r _ app_path _ <<< "$app_data"
            [[ "$requested_path" == "$app_path" ]] || continue
            local duplicate=false
            for selected_app in "${selected_apps[@]+"${selected_apps[@]}"}"; do
                IFS='|' read -r _ existing_path _ <<< "$selected_app"
                if [[ "$existing_path" == "$app_path" ]]; then
                    duplicate=true
                    break
                fi
            done
            [[ "$duplicate" == "true" ]] || selected_apps+=("$app_data")
            matched=true
            break
        done
        if [[ "$matched" == "false" ]]; then
            selected_apps=()
            echo "Selected application is no longer in the current scan: $requested_path" >&2
            return 1
        fi
    done
...
main() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            "--list")
                ...
                ;;
            "--gui-path="*)
                local gui_path="${arg#--gui-path=}"
                if [[ -z "$gui_path" || "$gui_path" == *$'\n'* || "$gui_path" == *$'\r'* || "$gui_path" != /* ]]; then
                    echo "Invalid native-app selection path" >&2
                    return 1
                fi
                gui_path_args+=("$gui_path")
                ;;
            "--whitelist")
    ...
    if [[ ${#gui_path_args[@]} -gt 0 && "${MOLE_DRY_RUN:-0}" != "1" && "${MOLE_GUI_CONFIRMED:-0}" != "1" ]]; then
        echo "Native-app confirmation is required before uninstalling applications" >&2
        return 1
    fi
    ...
    if [[ ${#app_name_args[@]} -gt 0 || ${#gui_path_args[@]} -gt 0 ]]; then
        local apps_file=""
        if ! apps_file=$(scan_applications); then
            uninstall_abort "could not complete the application scan"
            return 1
        ...
        if [[ ${#gui_path_args[@]} -gt 0 ]]; then
            if ! match_apps_by_gui_path "${gui_path_args[@]}"; then
                rm -f "$apps_file"
                echo "Application selection changed; rescan before uninstalling" >&2
                return 1
            fi
        else
        ...
        if [[ "${MOLE_GUI_CONFIRMED:-0}" != "1" && "${MOLE_DRY_RUN:-0}" != "1" ]]; then
            printf '\n'
