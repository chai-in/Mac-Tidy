...
# Serialize the exact review rows built by clean_project_artifacts. Bash uses
# dynamic scope here, so these names refer to that function's aligned local
# arrays. Output uses the original stdout saved by bin/purge.sh as fd 3.
mole_gui_print_purge_candidates() {
    MOLE_GUI_PURGE_JSON_EMITTED=1
    printf '[' >&3
    local i
    for ((i = 0; i < ${#item_paths[@]}; i++)); do
        [[ $i -gt 0 ]] && printf ',' >&3
        local recent="${item_recent_flags[i]:-true}"
        local cloud="${item_cloud_flags[i]:-false}"
        local unknown="${item_size_unknown_flags[i]:-false}"
        [[ "$recent" == "true" ]] || recent=false
        [[ "$cloud" == "true" ]] || cloud=false
        [[ "$unknown" == "true" ]] || unknown=false
        printf '{"path":"%s","display_path":"%s","project_path":"%s","artifact":"%s","size_kb":%s,"size_unknown":%s,"recent":%s,"age":"%s","cloud":%s}' \
            "$(mole_gui_json_escape "${item_paths[i]}")" \
            "$(mole_gui_json_escape "${item_display_paths[i]}")" \
            "$(mole_gui_json_escape "${item_project_paths[i]}")" \
            "$(mole_gui_json_escape "${item_paths[i]##*/}")" \
            "${item_sizes[i]:-0}" \
            "$unknown" \
            "$recent" \
            "$(mole_gui_json_escape "${item_age_labels[i]:-unknown}")" \
            "$cloud" >&3
    done
    printf ']\n' >&3
}
...
clean_project_artifacts() {
    ...
    if [[ "${MOLE_GUI_PURGE_MODE:-}" == "list" ]]; then
        mole_gui_print_purge_candidates
        PURGE_RUN_OUTCOME="gui_list"
        return 0
    fi
    ...
    if [[ "${MOLE_GUI_PURGE_MODE:-}" == "remove" ]]; then
        if [[ ${#MOLE_GUI_PURGE_PATHS[@]} -eq 0 ]]; then
            echo "No project artifacts selected" >&2
            PURGE_RUN_OUTCOME="selection_changed"
            return 1
        fi

        local requested_path selected_index
        local -a _mole_gui_selected_indices=()
        local selection_changed=false
        for requested_path in "${MOLE_GUI_PURGE_PATHS[@]}"; do
            local matched=false
            for ((i = 0; i < ${#item_paths[@]}; i++)); do
                [[ "$requested_path" == "${item_paths[i]}" ]] || continue
                local duplicate=false
                IFS=',' read -r -a _mole_gui_selected_indices <<< "$PURGE_SELECTION_RESULT"
                for selected_index in "${_mole_gui_selected_indices[@]+"${_mole_gui_selected_indices[@]}"}"; do
            ...
            if [[ "$matched" == "false" ]]; then
                echo "Selected artifact is no longer in the current safe plan: $requested_path" >&2
                selection_changed=true
            fi
        done
        if [[ "$selection_changed" == "true" ]]; then
            echo "Project artifact selection changed; rescan before purging" >&2
            PURGE_RUN_OUTCOME="selection_changed"
            return 1
        fi
    elif [[ -t 0 ]]; then
    ...
    if [[ -t 0 && "${MOLE_GUI_CONFIRMED:-0}" != "1" ]]; then
