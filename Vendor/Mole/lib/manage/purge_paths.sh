...
mole_gui_purge_paths_catalog() {
    ...
    printf '{"config_path":"%s","using_custom":%s,"paths":[' \
        "$(mole_gui_json_escape "$PURGE_PATHS_CONFIG")" "$custom"
    local i path exists
    for ((i = 0; i < ${#PURGE_SEARCH_PATHS[@]}; i++)); do
        [[ $i -gt 0 ]] && printf ','
        path="${PURGE_SEARCH_PATHS[i]}"
        exists=false
        [[ -d "$path" ]] && exists=true
        printf '{"path":"%s","exists":%s}' \
            "$(mole_gui_json_escape "$path")" "$exists"
    done
    printf '],"defaults":['
    for ((i = 0; i < ${#DEFAULT_PURGE_SEARCH_PATHS[@]}; i++)); do
        [[ $i -gt 0 ]] && printf ','
        printf '"%s"' "$(mole_gui_json_escape "${DEFAULT_PURGE_SEARCH_PATHS[i]}")"
    done
    printf ']}\n'
}

mole_gui_purge_paths_save() {
    local -a paths=()
    local path existing
    for path in "$@"; do
        if ! mole_gui_reject_multiline_value "$path"; then
            printf '{"saved":false,"error":"invalid empty or multiline path"}\n'
            return 1
        fi
        path="${path/#\~/$HOME}"
        if [[ "$path" != /* ]]; then
            printf '{"saved":false,"error":"scan paths must be absolute"}\n'
            return 1
        fi
...
# Delete all paths or this file to use defaults
" "${paths[@]+"${paths[@]}"}"; then
        printf '{"saved":false,"error":"could not write scan paths"}\n'
        return 1
    fi
