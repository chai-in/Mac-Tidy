...
source "$_MOLE_MANAGE_DIR/../core/gui.sh"
...
# Machine-readable inventory for the native app. The inventory and selected
# state come from the same helpers as the interactive manager.
mole_gui_whitelist_catalog() {
    local mode="${1:-clean}"
    local config_file="$WHITELIST_CONFIG_CLEAN"
    local items_source
    if [[ "$mode" == "optimize" ]]; then
        config_file="$WHITELIST_CONFIG_OPTIMIZE"
        ...
        printf '{"error":"invalid whitelist mode"}\n'
        return 1
    fi
    ...
    local -a patterns=()
    local -a categories=()
    local display_name pattern category
    ...
    local current_pattern predefined_pattern safety_pattern
    for current_pattern in "${CURRENT_WHITELIST_PATTERNS[@]+"${CURRENT_WHITELIST_PATTERNS[@]}"}"; do
        local known=false
        for predefined_pattern in "${patterns[@]+"${patterns[@]}"}"; do
        ...
        if [[ "$known" == "false" ]]; then
            for safety_pattern in "${SAFETY_WHITELIST_PATTERNS[@]+"${SAFETY_WHITELIST_PATTERNS[@]}"}"; do
        ...
        if [[ "$known" == "false" ]]; then
            custom_patterns+=("${current_pattern/#$HOME/~}")
        fi
    done

    printf '{"mode":"%s","config_path":"%s","items":[' \
        "$(mole_gui_json_escape "$mode")" \
        "$(mole_gui_json_escape "$config_file")"
    local i
    for ((i = 0; i < ${#patterns[@]}; i++)); do
        [[ $i -gt 0 ]] && printf ','
        local selected=false
        is_whitelisted "${patterns[i]}" && selected=true
        printf '{"name":"%s","pattern":"%s","category":"%s","selected":%s}' \
            "$(mole_gui_json_escape "${names[i]}")" \
            "$(mole_gui_json_escape "${patterns[i]/#$HOME/~}")" \
            "$(mole_gui_json_escape "${categories[i]}")" \
            "$selected"
    done
    printf '],"custom_patterns":['
    for ((i = 0; i < ${#custom_patterns[@]}; i++)); do
        [[ $i -gt 0 ]] && printf ','
        printf '"%s"' "$(mole_gui_json_escape "${custom_patterns[i]}")"
    done
    printf ']}\n'
}

# Replace user-editable selections atomically through Mole's existing writer.
# Mandatory safety patterns remain owned by load_whitelist and cannot be
# removed through this bridge.
mole_gui_whitelist_save() {
    local mode="${1:-clean}"
    shift || true
    if [[ "$mode" != "clean" && "$mode" != "optimize" ]]; then
        printf '{"saved":false,"error":"invalid whitelist mode"}\n'
        return 1
    fi
    ...
    local pattern
    for pattern in "$@"; do
        if ! mole_gui_reject_multiline_value "$pattern"; then
            printf '{"saved":false,"error":"invalid empty or multiline pattern"}\n'
            return 1
        fi
        patterns+=("${pattern/#$HOME/~}")
    done

    if [[ ${#patterns[@]} -gt 0 ]]; then
        save_whitelist_patterns "$mode" "${patterns[@]}"
    else
        save_whitelist_patterns "$mode"
    fi
    load_whitelist "$mode"
    printf '{"saved":true,"count":%d}\n' "${#patterns[@]}"
}
